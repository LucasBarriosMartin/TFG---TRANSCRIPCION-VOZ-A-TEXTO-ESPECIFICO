...
/*
 * Implementación del motor de reconocimiento usando Vosk.
 * Funciona completamente offline cargando un modelo local desde assets.
 * Se encarga de:
 * - Copiar el modelo a almacenamiento interno si es necesario
...
class Vosk(private val context: Context) : RecognitionEngine, RecognitionListener {
    ...
    override fun onPartialResult(hypothesis: String?) {
        ...
        if (texto.isNotEmpty()) {
            listener?.onResultadoParcial(texto)
        }
    }
    ...
    override fun onResult(hypothesis: String?) {
        ...
        if (texto.isNotEmpty()) {
            listener?.onResultadoFinal(texto)
        }
    }
